...
public class OxiaClientIT {
    //    @Container
    //    private static final OxiaContainer oxia = new
    // OxiaContainer(DEFAULT_IMAGE_NAME).withShards(4);
    //
    ...
    //        allOf(a, b, c, d).join();
    //
    //        assertThatThrownBy(() -> client.put("a", "a".getBytes(UTF_8),
    // IfRecordDoesNotExist).join())
    ...
    //        await()
    //                .untilAsserted(
    //                        () -> assertThat(notifications).contains(new KeyCreated("a",
    // finalAVersion)));
    //
    ...
    //        await()
    //                .untilAsserted(
    //                        () -> assertThat(notifications).contains(new KeyModified("a",
    // finalA2Version)));
    //
    //        // put with unexpected version
    //        var bVersion = client.get("b").join().getVersion().versionId();
    //        assertThatThrownBy(
    //                        () -> client.put("b", "b2".getBytes(UTF_8), ifVersionIdEquals(bVersion +
    // 1L)).join())
    ...
    //        assertThatThrownBy(
    //                        () -> client.delete("c", DeleteOption.ifVersionIdEquals(cVersion +
    // 1L)).join())
    //                .hasCauseInstanceOf(UnexpectedVersionIdException.class);
    //
    //        // list all keys
